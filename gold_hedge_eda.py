...
def analyze_option_implications(data, charts_dir):
    ...
    for level in strike_levels:
        ...
        # Estimate cost based on Black-Scholes approximation
        vol = gld_vol_252d.iloc[-1]  # Use latest volatility as proxy
        time_to_expiry = 1.0  # 1 year
        d1 = (np.log(latest_gld/strike) + (0.02 + 0.5 * vol**2) * time_to_expiry) / (vol * np.sqrt(time_to_expiry))
        d2 = d1 - vol * np.sqrt(time_to_expiry)
        
        from scipy.stats import norm
        put_price_estimate = strike * np.exp(-0.02 * time_to_expiry) * norm.cdf(-d2) - latest_gld * norm.cdf(-d1)
    ...
    ax2 = ax1.twinx()
    bars2 = ax2.bar(x + width/2, strike_df['Protection %'], width, color='blue', alpha=0.7, label='Historical Protection (%)')
    ax2.set_ylabel('Historical Protection (%)', color='blue')
    ax2.tick_params(axis='y', labelcolor='blue')
    
    # Add labels and title
    ax1.set_xticks(x)
    ax1.set_xticklabels([f"{level*100:.0f}% OTM\n(${strike_df['Strike Price'][i]:.2f})" for i, level in enumerate(strike_levels)])
...
def main():
    ...
    # Fetch data using FMP API
    data = fetch_data_fmp(api_key, start_date=args.start_date, end_date=args.end_date)
